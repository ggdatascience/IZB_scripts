...
pkg_nodig = c("shiny", "bslib", "tidyverse", "forecast", "this.path")
...
ui <- fluidPage(
    ...
    sidebarLayout(
        sidebarPanel(
            ...
            selectInput("inf", "Infectieziekte:", "Selecteer een databestand met het juiste format..."),
...
server <- function(input, output, session) {
  ...
  infections = reactive({
    data = read.csv(input$databestand)
    
    return(sort(unique(data$Infectieziekte)))
  })
  
  observe(updateSelectInput(session, "inf", choices=infections()))
  
  data = reactive({
    data = read.csv(input$databestand)
    data = data %>%
      rename(y=Jaar, m=Maandnummer) %>%
      filter(Infectieziekte == input$inf) %>%
      group_by(y, m) %>%
