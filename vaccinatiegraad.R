...
meta = cbs_get_meta("50136NED", catalog="RIVM") # 50136NED = 2024, 50117NED = 2023, 50093NED = 2022, 50082NED = 2021
...
data.rivm = cbs_get_data("50136NED", catalog="RIVM", RegioS=c("NL01  ", "GG1413", codes.nog$code))
data.rivm = data.rivm %>%
...
# geboortecohort berekenen
# de vaccinatielabels bevatten (x jaar), waardoor we jaar - leeftijd - 1 kunnen doen 
data.rivm = data.rivm %>%
  mutate(Leeftijd=as.numeric(str_match(Vaccinaties_label, "\\((\\d+) jaar\\)")[,2]),
         Cohort=Jaar-Leeftijd-1)
# MenACWY heeft een afwijkend label
data.rivm$Cohort[data.rivm$Vaccinaties == "A049175"] = data.rivm$Jaar[data.rivm$Vaccinaties == "A049175"] - 15 - 1

# convenience wrappers voor regio en NL
...
#kaartdata.gemeente = st_read("cbsgebiedsindelingen_2022_v1.gpkg", layer = "cbs_gemeente_2021_gegeneraliseerd")
...
save_img = function (name, vaccination, vaccination_hr, graphdata, fig.w, fig.h, size.factor) {
  # logische y-limiet uitrekenen
  # de meeste waardes zitten tussen 80 en 100
  if (min(graphdata[,c(1:3)], na.rm=T) > 80) ylim=c(75,100)
  else if (max(graphdata[,c(1:3)], na.rm=T) < 10) ylim=c(0,10) # voor geen deelname aan RVP
  else {
    range = pretty(graphdata[,c(1:3)])
    ylim = c(min(range), max(range))
  }
  
  png(sprintf("graphs/%s_%s.png", name, vaccination), width=fig.w, height=fig.h)
  par(cex.axis=size.factor/3, cex.lab=size.factor/3)
  
  # x labels moeten handmatig; we willen verslagjaar (geboortecohort)
  plot(graphdata[,4], graphdata[,1], col=nog_colors[1], type="l", lwd=size.factor,
       xlab=NA, ylab="Percentage", frame.plot=F, axes=F, main=sprintf("Vaccinatiegraad %s", vaccination_hr),
  ...
  plot(graphdata[,4], graphdata[,1], col=nog_colors[1], type="l", lwd=size.factor,
       ...
       ylim=ylim, cex=size.factor)
  ticks = axTicks(1)
  # we willen niet dat er 2020.5 op de as komt te staan, dus die maken we leeg
  ticks[!(ticks %in% graphdata[,4])] = ""
  # geboortecohorten toevoegen
  cohorten = ticks
  cohorten[cohorten != ""] = paste0("(", graphdata[graphdata[,4] %in% ticks[ticks != ""],5], ")")
  #ticks[ticks != ""] = sprintf("%s (%s)", ticks[ticks != ""], graphdata[graphdata[,4] %in% ticks[ticks != ""],5])
  axis(1, cex=size.factor, at=axTicks(1), labels=ticks)
  axis(1, cex=size.factor, at=axTicks(1), labels=cohorten, line=1, lwd=0, lwd.ticks=0, font=3)
  axis(2, cex=size.factor)
  # x-label iets verplaatsen om ruimte te maken voor de extra ticks
  mtext(expression(paste("Verslagjaar", italic("(geboortecohort)"))), side=1, line=4, cex=size.factor/3)
  
  lines(graphdata[,4], graphdata[,2], col=nog_colors[2], lwd=size.factor)
  lines(graphdata[,4], graphdata[,3], col=nog_colors[3], lwd=size.factor)
  ...
  legend("bottomright", legend=c(name, "NOG", "NL"), fill=nog_colors[1:3], inset=c(0.02, 0.02), cex=size.factor/3)
...
for (i in 1:nrow(codes.nog)) {
  ...
  graphdata = matrix(NA, nrow=nrow(data), ncol=5)
  colnames(graphdata) = c(naam, "Regio NOG", "Nederland", "Verslagjaar", "Cohort")
  ...
  graphdata[,4] = data$Jaar
  graphdata[,5] = data$Cohort
  
  save_img(naam, "DKTP_2jr", "DKTP (2 jaar)", graphdata, fig.w, fig.h, size.factor)
  ...
  save_img(naam, "BMR_2jr", "BMR (2 jaar)", graphdata, fig.w, fig.h, size.factor)
  ...
  save_img(naam, "MenACWY_2jr", "MenACWY (2 jaar)", graphdata, fig.w, fig.h, size.factor)
  ...
  save_img(naam, "nul_vacc_2jr", "Geen enkele vaccinatie (2 jaar)", graphdata, fig.w, fig.h, size.factor)
  ...
  save_img(naam, "vol_rvp_2jr", "Volledige deelname RVP (2 jaar)", graphdata, fig.w, fig.h, size.factor)
  ...
  save_img(naam, "DKTP_10jr", "DKTP (10 jaar)", graphdata, fig.w, fig.h, size.factor)
  ...
  save_img(naam, "BMR_10jr", "BMR (10 jaar)", graphdata, fig.w, fig.h, size.factor)
  ...
  save_img(naam, "HPV_14jr", "HPV (meisjes 14 jaar)", graphdata, fig.w, fig.h, size.factor)
  ...
  save_img(naam, "MenACWY_15jr", "MenACWY (15 jaar)", graphdata, fig.w, fig.h, size.factor)
}
...
# kaartjes van de laatste vaccinatiegraad van de regio waarin Yunio werkt (10 gemeentes)
for (vacc in vacc.types) {
  data.vacc = data.rivm %>%
    filter(Vaccinaties_label == vacc,
           RegioS %in% codes.nog$code[codes.nog$naam %in% gemeenten.yunio]) %>%
  ...
  png(sprintf("graphs/Yunio_kaart_%s_%d.png", str_replace_all(vacc, "\\W+", "_"), jaar), width=1400, height=1000)
  print(tm_shape(kaartdata.gemeente %>% right_join(data.vacc, by=c("statcode"="RegioS"))) +
...
for (vacc in vacc.types) {
  ...
  plot(data.vacc$Jaar[data.vacc$RegioS_label == "Nederland"], data.vacc$Vaccinatiegraad[data.vacc$RegioS_label == "Nederland"], col=nog_colors[1], type="l", lwd=2,
       xlab="Jaar (geboortecohort)", ylab=NA, frame.plot=F, ylim=ylim, axes=F,
       main=str_replace_all(vacc, c("TP voldoende beschermd"="TP kleuters", "TP volledig"="TP schoolkinderen", "BMR volledig"="BMR schoolkinderen")))
  grid(nx=NA, ny=NULL)
  ...
  ticks = pretty(data.vacc$Jaar, u5.bias=0.8)
  if (length(unique(data.vacc$Jaar)) <= 2)
    ticks = unique(data.vacc$Jaar)
  axis(1, at=ticks, labels=T)
  cohorten = paste0("(", unique(data.vacc$Cohort[data.vacc$Jaar %in% ticks]), ")")
  axis(1, at=unique(data.vacc$Jaar[data.vacc$Jaar %in% ticks]), labels=cohorten, line=1, lwd=0, lwd.ticks=0)
  axis(2)
  dev.off()
}
